...
7. Returns JSON including the local join descriptor, local port, and a generic invite command template.
...
```json
{
  ...
  "join_command_template": "parley join \"parley://127.0.0.1:49231/01j...\" --name <participant-name> --role <participant-role>",
...
The host does not know who will join, so invites must be generic. `parley invite` returns the current room descriptor and a ready-to-adapt command template, not a participant-specific command.
...
```json
{
  ...
  "local_port": 49231,
  "join_command_template": "parley join \"parley://127.0.0.1:49231/01j...\" --name <participant-name> --role <participant-role>",
  "agent_instruction": "Use your Parley skill to join this room: parley://127.0.0.1:49231/01j..."
}
```

Human handoff:

1. The host agent starts a room or runs `parley invite`.
2. The human copies the descriptor or `agent_instruction`.
3. The human pastes it into another agent session.
4. The joining agent uses its Parley skill to choose its own name and role, then runs a concrete join command such as:

```bash
parley join "parley://127.0.0.1:49231/01j..." --name codex-auth --role "auth reviewer"
```

The copied invite must not prescribe a participant name or role. Those values belong to the joining side.
...
The `--name` and `--role` values are supplied by the joining agent or by the human instruction given to that agent. They are not generated by the host invite.
...
```bash
parley join "parley://127.0.0.1:49231/01j..." --name codex-auth --role "auth reviewer" --dir /path --repo https://github.com/org/repo
```
...
- produce a generic invite with `parley invite`
- join a room from a pasted descriptor, choosing the joining participant's name and role locally
